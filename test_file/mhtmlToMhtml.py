...
def extract_links_from_mhtml(mhtml_path):
    """Extract Google Drive links directly from MHTML"""
    ...
    for part in msg.walk():
        if part.get_content_type() == 'text/html':
            ...
            for a in soup.find_all('a', href=True):
                ...
                if file_id:
                    filename = a.get_text(strip=True) or f"file_{file_id}"
                    target_links.append({
                        'id': file_id,
                        'url': href,
                        'name': filename,
...
def update_mhtml_links(original_path, url_mapping):
    """Update links in MHTML while preserving all original structure"""
    with open(original_path, 'rb') as f:
        msg = email.message_from_binary_file(f)
    
    # Create a deep copy of the original message
    new_msg = email.message_from_bytes(msg.as_bytes())
    
    # Find and update the HTML part
    for part in new_msg.walk():
        if part.get_content_type() == 'text/html':
            html_content = part.get_payload(decode=True).decode('utf-8', errors='replace')
            soup = BeautifulSoup(html_content, 'html.parser')
            ...
            # Update the part with modified content
            updated_html = str(soup)
            part.set_payload(updated_html, 'utf-8')
...
def main():
    ...
    # Extract links directly from MHTML
    all_file_infos = extract_links_from_mhtml(MHTML_FILE)
    ...
    if processed_count > 0:
        ...
        new_mhtml_path = update_mhtml_links(MHTML_FILE, url_mapping)
